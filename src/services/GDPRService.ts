...
export type ActivitiesGDPRResponse = Pick<
  BaseActivity,
  'createdAt' | 'updatedAt' | 'deletedAt' | 'type' | 'descriptionDutch' | 'descriptionEnglish'
>;
export type FileGDPRResponse = Pick<
  BaseFile,
  'createdAt' | 'updatedAt' | 'deletedAt' | 'name' | 'downloadName' | 'location'
>;
...
export interface UserGDPRDumpResponse
  extends Pick<
    User,
    'firstName' | 'lastNamePreposition' | 'lastName' | 'gender' | 'function' | 'email' | 'replyToEmail'
  > {
...
export default class GDPRService {
  ...
  private toProductInstanceGDPRResponse(
    instance: ProductInstance,
    activitiesMap: Map<typeof BaseActivity, BaseActivity[]>,
  ...
  async getDump(id: number): Promise<UserGDPRDumpResponse> {
    ...
    await Promise.all(
      [CompanyActivity, ContractActivity, InvoiceActivity, ProductActivity, ProductInstanceActivity].map(async (c) => {
        const activities = await AppDataSource.getRepository(c).find({
    ...
    await Promise.all(
      [CompanyFile, ContractFile, InvoiceFile, ProductFile].map(async (c) => {
    ...
    const productInstanceIds = ((activitiesMap.get(ProductInstanceActivity) || []) as ProductInstanceActivity[]).map(
      (a) => a.productInstanceId,
    );
    const productInstances = await AppDataSource.getRepository(ProductInstance).find({
      where: { id: In(this.remDupes(productInstanceIds)) },
    ...
    const contracts = await AppDataSource.getRepository(Contract).find({
      where: [{ id: In(this.remDupes(contractIds)) }, { createdById: user.id }, { assignedToId: user.id }],
      withDeleted: true,
    });
    ...
    const invoices = await AppDataSource.getRepository(Invoice).find({
      where: [{ id: In(this.remDupes(invoiceIds)) }, { createdById: user.id }, { assignedToId: user.id }],
      withDeleted: true,
    });
    ...
    const products = await AppDataSource.getRepository(Product).find({
      where: { id: In(this.remDupes(productIds)) },
      withDeleted: true,
    });
    ...
    const companies = await AppDataSource.getRepository(Company).find({
      where: { id: In(this.remDupes(companyIds)) },
      withDeleted: true,
    });
